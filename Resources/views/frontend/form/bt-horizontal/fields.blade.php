    ...
    @foreach($fields as $index => $field)
        <div class="form-group">
            @if($field['type'] != "textarea" && $field['type'] != "select" && $field['type'] != "checkbox" && $field['type'] != "radio")
                <label for="{!!$field['name']!!}" class="col-sm-2 control-label">{{ trans('icustom::iforms.field.'.$field['name']) }} </label>
                <div class="col-sm-10">
            ...
            @elseif($field['type'] == "textarea")
                <label for="{!!$field['name']!!}" class="control-label col-sm-2">{{ trans('icustom::iforms.field.'.$field['name'])}}</label>
                <div class="col-sm-10">
                    <textarea class="form-control" name="{!!$field['name']!!}" placeholder="{{ trans('icustom::iforms.field.'.$field['description']) }}" rows="4"></textarea>
                </div>
    ...
    @foreach($fields as $index => $field)
        <div class="form-group">
            ...
            @elseif($field['type'] == "textarea")
                ...
                <div class="col-sm-10">
                    <textarea class="form-control" name="{!!$field['name']!!}" placeholder="{{ $field['description'] }}" rows="4"></textarea>
                </div>
            @elseif($field['type'] == "select")
            @elseif($field['type'] == "checkbox"||$field['type'] == "radio")
                <div class="{!!$field['type']!!}">
